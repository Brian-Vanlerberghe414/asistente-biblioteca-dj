"""Búsqueda de previews en YouTube para los tracks de los charts (Módulo 2).

Sin API key — usa yt-dlp en modo búsqueda (`ytsearchN:...`), en el mismo
espíritu que `charts_beatport.py` lee Beatport sin credenciales.

Los charts de Beatport casi siempre traen la versión "Extended Mix"; YouTube
en cambio suele tener la versión corta/radio. Por eso esta búsqueda prioriza
...
def buscar_candidatos(
    ...
    varios candidatos (no solo el mejor) permite que la GUI pruebe el
    ...
    red, o no se encuentra nada con confianza suficiente.
    """
    if not titulo:
        return []
...
def buscar(artistas: list[str], titulo: str, mix_name: Optional[str] = None) -> Optional[ResultadoYoutube]:
    """Atajo: devuelve solo el mejor candidato (ver `buscar_candidatos`)."""
    candidatos = buscar_candidatos(artistas, titulo, mix_name, n=1)
